...
//almacen
    ...
    public static String BuscaProyectoCompras = "SELECT * FROM tb_proyectos where F_id not in (0);";
    ...
    public static String ExistenciaProyectoCompraGlobal = "SELECT P.F_DesProy, L.F_ClaPro, SUBSTR(M.F_DesPro,1,40) AS F_DesPro, L.F_ClaLot, DATE_FORMAT(L.F_FecCad, '%d/%m/%Y') AS F_FecCad, SUM(L.F_ExiLot) AS F_ExiLot, O.F_DesOri, L.F_Ubica FROM tb_lote L INNER JOIN tb_medica M ON L.F_ClaPro = M.F_ClaPro INNER JOIN tb_origen O ON L.F_Origen = O.F_ClaOri INNER JOIN tb_proyectos P ON L.F_Proyecto = P.F_Id WHERE L.F_Proyecto = ? AND L.F_ExiLot > 0 AND L.F_ExiLot > 0 AND F_Ubica  NOT RLIKE 'CADUCADOS|MERMA|EXTRA_ORDINARIA|MRM|CDC' GROUP BY L.F_ClaPro, L.F_Proyecto;";
    ...
    public static String ContarClaveProyectoCompra = "SELECT COUNT(DISTINCT(F_ClaPro)) AS CONTAR FROM tb_lote L WHERE L.F_Proyecto = ? AND L.F_ExiLot > 0 AND F_Ubica NOT RLIKE 'CADUCADOS|MERMA|EXTRA_ORDINARIA|MRM|CDC' ;";

    public final String ExistenciaDisp = "SELECT p.F_DesProy AS F_DesProy, l.F_ClaPro AS F_ClaPro, substr(m.F_DesPro, 1, 40) AS F_DesPro, l.F_ClaLot AS F_ClaLot, date_format(l.F_FecCad, '%d/%m/%Y') AS F_FecCad, sum(l.F_ExiLot) AS F_ExiLot, o.F_DesOri AS F_DesOri, l.F_Ubica AS F_Ubica, l.F_Proyecto AS F_Proyecto, substr(m.F_PrePro, 1, 40) AS F_PrePro FROM tb_lote l INNER JOIN tb_medica m ON l.F_ClaPro = m.F_ClaPro INNER JOIN tb_origen o ON l.F_Origen = o.F_ClaOri INNER JOIN tb_proyectos p ON l.F_Proyecto = p.F_Id WHERE l.F_Proyecto = ? AND l.F_ExiLot > 0 AND l.F_Ubica NOT RLIKE 'CADUCADOS|MERMA|EXTRA_ORDINARIA|MRM|CDC|NUEVA' GROUP BY l.F_ClaPro, l.F_ClaLot, l.F_FecCad, l.F_Proyecto HAVING F_ExiLot > 0;";

    public final String ExistenciaDispT = "SELECT p.F_DesProy AS F_DesProy, l.F_ClaPro AS F_ClaPro, substr(m.F_DesPro, 1, 40) AS F_DesPro, l.F_ClaLot AS F_ClaLot, date_format(l.F_FecCad, '%d/%m/%Y') AS F_FecCad, sum(l.F_ExiLot) AS F_ExiLot, o.F_DesOri AS F_DesOri, l.F_Ubica AS F_Ubica, l.F_Proyecto AS F_Proyecto, substr(m.F_PrePro, 1, 40) AS F_PrePro FROM tb_lote l INNER JOIN tb_medica m ON l.F_ClaPro = m.F_ClaPro INNER JOIN tb_origen o ON l.F_Origen = o.F_ClaOri INNER JOIN tb_proyectos p ON l.F_Proyecto = p.F_Id WHERE l.F_Proyecto IN (%s) AND l.F_ExiLot > 0 AND l.F_Ubica NOT RLIKE 'CADUCADOS|MERMA|EXTRA_ORDINARIA|MRM|CDC|NUEVA' GROUP BY l.F_ClaPro, l.F_ClaLot, l.F_FecCad, l.F_Proyecto HAVING F_ExiLot > 0;";
...
//clientes  
    ...
    public static String ContarClaveTodosCompra = "SELECT COUNT(DISTINCT(F_ClaPro)) AS CONTAR FROM tb_lote L WHERE L.F_ExiLot > 0 AND F_Ubica  NOT RLIKE 'CADUCADOS|MERMA|EXTRA_ORDINARIA|MRM|CDC';";
