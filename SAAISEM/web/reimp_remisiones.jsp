    ...
    <body>
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-sm-2">
                    <br/>
                    <a class="btn btn-primary" href="gnrFacturaConcentrado.jsp?f1=<%=Fecha1%>&f2=<%=Fecha2%>&proyecto=<%=proyecto%>" target="_blank">Exportar Global</a>
                </div>
                <%}%>
            </div>
            <form name="forma1" id="forma1" action="AdministraRemisiones" method="post">
                ...
                <div>
                    ...
                    <div class="panel panel-primary">
                        <div class="panel-body table-responsive" style="overflow:scroll; width:100%;" >
                            <table class="table table-bordered table-striped" id="datosCompras">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Fec Ent</th>
                                        <th>Tipo</th>
                                        <%if (tipo.equals("1") || tipo.equals("7") || usua.equals("silvinomm")) {%>
                                        <th>Folio</th>
                                            <%}%>
                                        <th>Excel</th> 
                                        <%if (tipo.equals("1") || tipo.equals("21") ) {%>
                                        <th>Cancelados</th>
                                         <%}%>
                                         <%if (tipo.equals("1") || tipo.equals("7") || tipo.equals("21") ) {%>
                                        <th>Abasto</th>
                                         <%}%>
                                    ...
                                    <c:forEach items="${listaRemision}" var="detalle">
                                        <tr>
                                            ...
                                            <td><c:out value="${detalle.tipofact}" /></td>
                                             <%if (tipo.equals("1") || tipo.equals("7") || tipo.equals("13") || usua.equals("silvinomm")) {%>
                                            <td>
                                                <c:if test="${detalle.ver == 'SI'}">
                                                    <a href="ImprimeFolio?fol_gnkl=<c:out value="${detalle.folio}" />&Proyecto=<c:out value="${detalle.idproyecto}" />&BanDato=<c:out value="${detalle.ban}" />&idProyecto=<c:out value="${detalle.idproyecto}" />&controlado=<c:out value="${detalle.controlado}"/>" target="_blank" class="btn btn-block btn-primary"><span class="glyphicon glyphicon-print"></span></a>
                                                    </c:if>
                                            </td>   
                                             <%}%>
                                            <!--td>
                                                <//c:if test="{(detalle.usuario == 'sistemas')  || (detalle.usuario == 'Francisco')  || (detalle.usuario == 'Rosalino')}">                                            
                                                        <input class="hidden" name="fol_gnkl" value="<//c:out value="{detalle.folio}" />">
                                                        <button class="btn btn-block btn-info" name="Accion" value="DevoGlobal"><span class="glyphicon glyphicon-arrow-left"></span></button>                                            
                                                <///c:if>                                        
                                            </td-->

                                            <td>
                                                <c:if test="${detalle.ver == 'SI'}">
                                                    <a class="btn btn-block btn-primary" href="gnrFacturaExcel.jsp?fol_gnkl=<c:out value="${detalle.folio}" />&idProyecto=<c:out value="${detalle.idproyecto}" />"><span class="glyphicon glyphicon-save"></span></a>
                                                    </c:if>
                                            </td>
                                            ...
                                            <c:if test="${detalle.sts == 'C'}">
                                                       <a style="width: 60px;margin: 0 auto;" class="btn btn-info btn-block" target="_blank" onclick="javascript:window.open('canceladosFacturacion.jsp?F_ClaDoc=<c:out value="${detalle.folio}"/>&ConInv=<c:out value="${detalle.claveuni}"/>&idProyecto=<c:out value="${detalle.idproyecto}" />&fentrega=<c:out value="${detalle.fechae}"/>&ubi=<c:out value="${detalle.ubi}"/>&folio1=78800&folio2=78800&fecha_ini=2021-12-15&fecha_fin=2021-12-15&Proyecto=0&Accion=mostrar&datosCompras_length=10/','','width=1000,height=600,left=80,top=100,toolbar=yes');" > <span  class="glyphicon glyphicon-remove-sign"></span></a> 
                                                  </c:if>
                                            </td>
                                            <%}%>
                                            
                                            <%if (tipo.equals("1") || tipo.equals("7") || tipo.equals("21") || tipo.equals("13") ) {%>
                                            <td>
                                                
                                                <c:if test="${detalle.ver == 'SI'}">
                                                    <a class="btn btn-info btn-block" target="_black" href="facturacion/generaAbastoCSV.jsp?F_ClaDoc=<c:out value="${detalle.folio}"/>&ConInv=<c:out value="${detalle.claveuni}"/>&idProyecto=<c:out value="${detalle.idproyecto}" />"><span class="glyphicon glyphicon-download"></span></a>
                                                    <br/>
                                         ...
                                         <%if (tipo.equals("1") || tipo.equals("7")) {%>
                                            ...
                                            <td>
                                                <c:if test="${detalle.cancela == 'SI'}">
                                                    <a class="btn btn-block btn-info" data-toggle="modal" data-target="#Cancelacion<c:out value="${detalle.folio}" /><c:out value="${detalle.idproyecto}" />"><span class="glyphicon glyphicon-trash"></span></a>                                   
                                                    </c:if>
                                            </td>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="modal-footer">
                                        <button type="submit" class="btn btn-primary" id="<c:out value="${detalle.folio}" /><c:out value="${detalle.idproyecto}" />" disabled onclick="return validaCancel(this.id);" name="accion" value="CancelarFolio">CONFIMAR CANCELACION</button>
