...
public class ExcelExistenciaDisponible extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (Proyecto.equals("0")) {
                qry = "SELECT p.F_DesProy AS F_DesProy, l.F_ClaPro AS F_ClaPro, substr(m.F_DesPro, 1, 40) AS F_DesPro, l.F_ClaLot AS F_ClaLot, date_format(l.F_FecCad, '%d/%m/%Y') AS F_FecCad, sum(l.F_ExiLot) AS F_ExiLot, o.F_DesOri AS F_DesOri, l.F_Ubica AS F_Ubica, l.F_Proyecto AS F_Proyecto FROM tb_lote l INNER JOIN tb_medica m ON l.F_ClaPro = m.F_ClaPro INNER JOIN tb_origen o ON l.F_Origen = o.F_ClaOri INNER JOIN tb_proyectos p ON l.F_Proyecto = p.F_Id WHERE l.F_Proyecto = ? AND l.F_ExiLot > 0  AND l.F_ExiLot > 0 AND l.F_Ubica NOT RLIKE 'CADUCADOS|MERMA|EXTRA_ORDINARIA|MRM|CDC|NUEVA' GROUP BY l.F_ClaPro, l.F_Proyecto;";
                Proyecto = ProyectoL;
            } else {
                qry = "SELECT p.F_DesProy AS F_DesProy, l.F_ClaPro AS F_ClaPro, substr(m.F_DesPro, 1, 40) AS F_DesPro, l.F_ClaLot AS F_ClaLot, date_format(l.F_FecCad, '%d/%m/%Y') AS F_FecCad, sum(l.F_ExiLot) AS F_ExiLot, o.F_DesOri AS F_DesOri, l.F_Ubica AS F_Ubica, l.F_Proyecto AS F_Proyecto, substr(m.F_PrePro, 1, 40) AS F_PrePro FROM tb_lote l INNER JOIN tb_medica m ON l.F_ClaPro = m.F_ClaPro INNER JOIN tb_origen o ON l.F_Origen = o.F_ClaOri INNER JOIN tb_proyectos p ON l.F_Proyecto = p.F_Id WHERE l.F_Proyecto = ? AND l.F_ExiLot > 0 AND l.F_Ubica NOT RLIKE 'CADUCADOS|MERMA|EXTRA_ORDINARIA|MRM|CDC|NUEVA' GROUP BY l.F_ClaPro, l.F_ClaLot, l.F_FecCad, l.F_Proyecto HAVING F_ExiLot > 0;";
            }
            aux = 0;
