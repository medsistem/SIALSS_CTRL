...
<% /*Parametros para realizar la conexión*/
    ...
    String DesV = "", remis = "", ProyectoF = "", ProyectoFactura = "", Nomenclatura = "", Encabezado = "", RedFria = "";
    ...
    ResultSet FoliosC = con.consulta("SELECT F.F_Folio, F_Copy, U.F_Proyecto, F.F_Proyecto AS F_ProyectoF FROM tb_folioimp F INNER JOIN tb_enseresfactura FT ON F.F_Folio = FT.F_Folio AND F.F_Proyecto = FT.F_Proyecto INNER JOIN tb_uniatn U ON FT.F_ClaCli = U.F_ClaCli WHERE F.F_User = '" + User + "' GROUP BY F.F_Folio, F.F_Proyecto ORDER BY F.F_Folio + 0;");
    while (FoliosC.next()) {
        ...
        while (DatosFactMed.next()) {
            ...
            con.actualizar("INSERT INTO tb_enseresimprefolio VALUES('" + DatosFactMed.getString(1) + "','" + DatosFactMed.getString(2) + "','" + DatosFactMed.getString(3) + "','" + DatosFactMed.getString(4) + "','" + DatosFactMed.getString(5) + "','" + DatosFactMed.getString(6) + "','" + DatosFactMed.getString(7) + "','" + DatosFactMed.getString(8) + "','" + DatosFactMed.getString(9) + "','" + DatosFactMed.getString(10) + "','" + DatosFactMed.getString(11) + "','" + DatosFactMed.getString(12) + "','" + DatosFactMed.getString(13) + "','" + DatosFactMed.getString(14) + "','" + DatosFactMed.getString(15) + "','" + DatosFactMed.getString(16) + "','" + usua + "','"+remis+"',0);");
        }
        if (SumaMedSurT > 0) {
            con.actualizar("INSERT INTO tb_enseresimprefolio VALUES('','" + Folio + "','','" + Unidad + "','" + Direc + "','" + Razon + "','" + Fecha + "','" + F_FecApl + "','','Total Enseres','" + SumaMedSurT + "','" + Jurisdiccion + "','" + Municipio + "','" + Clues + "','" + F_Obs + "','" + Proyecto + "','" + usua + "','"+remis+"',0);");
        }
        ...
        if (Ban == 1) {
            for (int x = 0; x < Copy; x++) {
                ...
                File reportFile;
                reportFile = new File(application.getRealPath("/reportes/EnseresFolio.jasper"));
            ...
            for (int x = 0; x < Copy; x++) {
                ...
                reportFile = new File(application.getRealPath("/reportes/EnseresFolio2.jasper"));
