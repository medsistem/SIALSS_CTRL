                    ...
                    <form class="form-horizontal" role="form" name="formulario1" id="formulario1" method="post" action="medicamentoUnidad.jsp">
                        <div class="row">
                            <div class="form-group">
                                    ...
                                    <select name="SelectUnidad" id="SelectUnidad" class="form-control">
                                        ...
                                        <%
                                            try {
                                                con.conectar();
                                                ResultSet rset = null;
                                                rset = con.consulta("SELECT M.F_ClaUni, U.F_NomCli FROM tb_medicaunidad M INNER JOIN tb_uniatn U ON M.F_ClaUni = U.F_ClaCli WHERE M.F_Autorizado != 2 AND U.F_Proyecto = 2 GROUP BY M.F_ClaUni;");
                                                while (rset.next()) {
                                        %>
                                ...
                                <div class="col-sm-2">
                                    <button name="btnUnidad" id="btnUnidad" class="btn btn-block btn-info form-control">Por Unidad</button>
                                </div>
                                <%if (!(SelectUnidad.equals("0"))) {%>
                                <div class="col-sm-2">
                                    <a href="gnrMedUnidad.jsp?Unidad=<%=SelectUnidad%>" target="_black" class="btn btn-block btn-primary form-control">Exportar</a>
                                </div>
                                <%}%>
                            </div>
                        </div>
