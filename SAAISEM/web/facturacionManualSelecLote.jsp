...
<%
    ...
    String ClaCli = "",ClaCli1 = "", FechaEnt = "", ClaPro = "", DesPro = "", Cantidad = "", DesProyecto = "", Proyecto = "", clacliente = "";

    try {
        
        ClaCli = (String) sesion.getAttribute("ClaCliFM");
        ClaCli1 = (String) request.getParameter("ClaCliente");
   ...
   // String clacliente = ClaCli.substring(0,5);
%>
<html>
    <head>
        ...
        <div class="container">
            ...
            <form action="FacturacionManual" method="post">
                <div class="row">
                    ...
                    <div class="col-sm-4">
                        <input value="<%=clacliente%>"  class="form-control" name="ClaCli" id="ClaCli" readonly="" />
            ...
            <table class="table table-condensed table-striped table-bordered table-responsive">
                ...
                <%
                    try {
                        ...
                        String Ubicaciones = "", UbicaNofacturar = "", id = "", idubica = "";
                        String qryUbicaDesc = "SELECT UF.F_idUbicaFac, UF.F_UbicaSQL2,UF.F_idUbicaFac FROM tb_parametrousuario AS PU INNER JOIN tb_ubicafact AS UF ON PU.F_Id = UF.F_idUbicaFac WHERE PU.F_Usuario = '" + usua + "' ";
                        ResultSet rsetR2 = con.consulta(qryUbicaDesc);
                        while (rsetR2.next()) {
                            id = rsetR2.getString(1);
                            Ubicaciones = rsetR2.getString(2);
                            idubica = rsetR2.getString(3);
                        }%>
<tr>
                ...
                <%
                        if (Proyecto.equals("2")) {
                            ...
                            String filtroOrigen = "", filtroRural = "", tipoUnidad = "";

                            ResultSet rsetN;
                            rsetN = con.consulta("Select F_Nivel, F_Tipo from tb_uniatn where F_ClaCli = '" + clacliente + "'");
                            while (rsetN.next()) {
                                Nivel = rsetN.getInt(1);
                                tipoUnidad = rsetN.getString(2);
                                System.out.println("Nivel es :" + Nivel);
                                System.out.println("Tipo es :" + tipoUnidad);
                            } //termina el ciclo     
                            if (id.equals("8")) {
                            ...
                            ResultSet rsetN3;
                            if (id.equals("11")) {
                                rsetN3 = con.consulta("Select * from tb_unidgastoscata ugc where ugc.ClaCli = '" + clacliente + "' and ugc.Sts_Cliente = '1' ");
                            ...
                            switch (id) {
                                case "6":
                                    ...
                                    if (Nivel == 1) {
                                        ...
                                        //  rset = con.consulta("SELECT l.F_ClaPro, l.F_ClaLot, DATE_FORMAT(l.F_FecCad, '%d/%m/%Y'), l.F_Ubica, (d.disponible) as F_ExiLot, l.F_IdLote, l.F_FolLot, m.F_DesMar, d.F_DesOri, p.F_DesProy FROM tb_lote l LEFT JOIN v_existencias d ON l.F_IdLote = d.F_IdLote INNER JOIN tb_marca m ON l.F_ClaMar = m.F_ClaMar INNER JOIN tb_medica me ON me.F_ClaPro = l.F_ClaPro INNER JOIN tb_proyectos p ON l.F_Proyecto = p.F_Id " + Ubicaciones + " AND l.F_ClaPro = '" + ClaPro + "' AND l.F_ExiLot != 0 AND l.F_Proyecto = '" + Proyecto + "' " + filtroOrigen + filtroRural + " HAVING F_ExiLot > 0 ORDER BY l.F_Origen, l.F_FecCad ASC;");
                                ...
                                case "9":
                                    ...
                                    break;
                                        ...
                                        case "11":
                                            ...
                                            System.out.println("query de Gastos: " + rset);

                                            break;
                                            
                                     case "12":
                                            System.out.println("SSM FACTURA MANUAL");
                                         
                                                filtroRural = " AND l.F_FecCad > CURDATE()";
                                        rset = con.consulta("SELECT l.F_ClaPro,l.F_ClaLot,DATE_FORMAT( l.F_FecCad, '%d/%m/%Y' ),l.F_Ubica,l.disponible,l.F_IdLote,l.F_FolLot,l.F_DesMar,l.F_DesOri,l.F_DesProy FROM v_existencias AS l " + Ubicaciones + " AND l.F_ClaPro = '" + ClaPro + "'  AND l.disponible > 0 AND l.F_Proyecto = '" + Proyecto + "' " + filtroRural + " ORDER BY l.F_FecCad ASC,l.F_ClaLot ASC, l.F_DesOri ASC;");
                               ...
                               break;
                                ...
                                default:
                                    ...
                                    if (!tipoUnidad.equals("1")) {
                                        ...
                                        out.println("<script>alert('Validar Parametro de Facturacion para 2 y 3 nivel')</script>");
                                         //   request.getRequestDispatcher("facturacionManual.jsp").forward(request, response);
                                            // response.sendRedirect("facturacionManual.jsp");
   
                                    }
                                               System.out.println("query default: " + rset);
                            }

                        }//fin del if proyecto Mich
...
//    rset = con.consulta("SELECT l.F_ClaPro, l.F_ClaLot, DATE_FORMAT(l.F_FecCad, '%d/%m/%Y'), l.F_Ubica, l.F_ExiLot, l.F_IdLote, l.F_FolLot, m.F_DesMar, l.F_Origen, p.F_DesProy FROM tb_lote l INNER JOIN tb_marca m ON l.F_ClaMar = m.F_ClaMar INNER JOIN tb_medica me ON me.F_ClaPro = l.F_ClaPro INNER JOIN tb_proyectos p ON l.F_Proyecto = p.F_Id WHERE l.F_ClaPro = '" + ClaPro + "' AND F_ExiLot != 0 AND l.F_Proyecto = '" + Proyecto + "' AND l.F_Ubica NOT IN (" + Ubicaciones + ",'CUARENTENA') ORDER BY l.F_Origen, F_FecCad ASC;");
                            rset = con.consulta("SELECT l.F_ClaPro, l.F_ClaLot, DATE_FORMAT(l.F_FecCad, '%d/%m/%Y'), l.F_Ubica, (d.disponible) as F_ExiLot, l.F_IdLote, l.F_FolLot, m.F_DesMar, d.F_DesOri, p.F_DesProy FROM tb_lote l LEFT JOIN v_existencias d ON l.F_IdLote = d.F_IdLote INNER JOIN tb_marca m ON l.F_ClaMar = m.F_ClaMar INNER JOIN tb_medica me ON me.F_ClaPro = l.F_ClaPro INNER JOIN tb_proyectos p ON l.F_Proyecto = p.F_Id " + Ubicaciones + " AND l.F_ClaPro = '" + ClaPro + "' AND l.F_ExiLot != 0 AND l.F_Proyecto = '" + Proyecto + "'  HAVING F_ExiLot > 0 ORDER BY l.F_Origen, l.F_FecCad ASC;");
//                            rset = con.consulta("SELECT l.F_ClaPro, l.F_ClaLot, DATE_FORMAT(l.F_FecCad, '%d/%m/%Y'), l.F_Ubica, (d.disponible) as F_ExiLot, l.F_IdLote, l.F_FolLot, m.F_DesMar, d.F_DesOri, p.F_DesProy FROM tb_lote l LEFT JOIN v_existencias d ON l.F_IdLote = d.F_IdLote INNER JOIN tb_marca m ON l.F_ClaMar = m.F_ClaMar INNER JOIN tb_medica me ON me.F_ClaPro = l.F_ClaPro INNER JOIN tb_proyectos p ON l.F_Proyecto = p.F_Id " + Ubicaciones + " AND l.F_ClaPro = '" + ClaPro + "' AND l.F_ExiLot != 0 AND l.F_Proyecto = '" + Proyecto + "' AND l.F_FecCad > DATE_ADD(NOW(),INTERVAL 7 DAY) HAVING F_ExiLot > 0 ORDER BY l.F_Origen, l.F_FecCad ASC;");
...
//                            rset = con.consulta("SELECT l.F_ClaPro, l.F_ClaLot, DATE_FORMAT(l.F_FecCad, '%d/%m/%Y'), l.F_Ubica, (d.disponible) as F_ExiLot, l.F_IdLote, l.F_FolLot, m.F_DesMar, d.F_DesOri, p.F_DesProy FROM tb_lote l LEFT JOIN v_existencias d ON l.F_IdLote = d.F_IdLote INNER JOIN tb_marca m ON l.F_ClaMar = m.F_ClaMar INNER JOIN tb_medica me ON me.F_ClaPro = l.F_ClaPro INNER JOIN tb_proyectos p ON l.F_Proyecto = p.F_Id " + Ubicaciones + " AND l.F_ClaPro = '" + ClaPro + "' AND l.F_ExiLot != 0 AND l.F_Proyecto = '" + Proyecto + "' AND l.F_FecCad > DATE_ADD(NOW(),INTERVAL 7 DAY) HAVING F_ExiLot > 0 ORDER BY l.F_Origen, l.F_FecCad ASC;");
                    ...
                    System.out.println("query: " + rset);
                        ...
                        while (rset.next()) {
                            ...
                            System.out.println("Cantidad: " + cant + " " + cantTemp);

// if (cant <= 0) {
//System.out.println("tenemos problemas rayli!");
//        cant = 0;
// response.sendRedirect("facturacionManual.jsp");
//        out.print("<script> alert('No hay existencia','Error') </script>");
//     }
                ...
                <tr>
                    ...
                    <td>
                        <form action="FacturacionManual" method="post">
                            ...
                            <% if (id.equals("11")) {%>
                            <input class="hidden" name="Cant" id="Cant<%=rset.getString(11)%>" value=""/>
