                    ...
                    <%
                        try {
                            ...
                            try {
                                ...
                                ResultSet rset = con.consulta("SELECT M.F_Clapro,MD.F_DesPro,M.F_Min,M.F_Reo,M.F_Max,M.F_Ubi FROM tb_minmax M INNER JOIN tb_medica MD ON M.F_Clapro=MD.F_ClaPro WHERE M.F_Ubi NOT IN ('D','MODULA2','AF','MATCUR','CADUCADOS','PROXACADUCAR','MERMA','CROSSDOCKMORELIA','INGRESOS_V','CUARENTENA','FALTANTE_ORIGEN','EXTRA_ORDINARIA') ORDER BY M.F_Clapro+0;");
                                while (rset.next()) {
                                    F_Max = rset.getInt(5);
                                    F_Reo = rset.getInt(4);
                                    F_Min = rset.getInt(3);
                                    ResultSet ExiMod = con.consulta("SELECT SUM(F_ExiLot) FROM tb_lote WHERE F_ClaPro='" + rset.getString(1) + "' AND F_Ubica='" + rset.getString(6) + "' AND F_Proyecto = 2;");
                                    if (ExiMod.next()) {
