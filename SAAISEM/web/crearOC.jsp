        ...
        <div class="container">
            ...
            <div class="row">
                <%                    try {
                        ...
                        String Class = "", IdOC = "";
                        int Contar = 0, Ban = 0;
                        ResultSet rset = con.consulta("SELECT F_Id, F_DesProy, F_Campo, F_NomOC, F_IdOC FROM tb_proyectos  WHERE F_Id IN (" + Proyecto + ") ORDER BY F_DesProy ASC;");
                        while (rset.next()) {
                            Contar++;
                            IdOC = rset.getString(5);
                            if (Contar == 1) {
                                Class = "btn-info";
                            } else if (Contar == 2) {
                                Class = "btn-info";
                ...
                %>
                <div class="col-sm-3">
                    <a href="capturaMedalfa.jsp?Proyecto=<%=rset.getString(1)%>&DesProyecto=<%=rset.getString(2)%>&Campo=<%=rset.getString(3)%>&NomOC=<%=rset.getString(4)%>&IdOC=<%=rset.getString(5)%>&Ban=<%=Ban%>" class="btn btn-block form-control <%=Class%>">
                        <%=rset.getString(2)%></a>
                </div>
                <br />
                <%if (Contar > 4) {%>
